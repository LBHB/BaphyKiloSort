...
loadcmd='/home/nems/nems_db/bin/qsetload';
...
if hostislocal
   ...
   shorthostname=compdata.name;
   tuser='nems@'; % [user '@'];
   disp([SSHCMD,tuser,shorthostname,' ',loadcmd,bgstring]);
   [s,w]=unix([SSHCMD,tuser,shorthostname,' ',loadcmd,bgstring]);
   
   if s>0
      fprintf('dbupdateload.m: s=%d re-checking load on: %s\n',s,hostname);
      [s,w]=unix([SSHCMD,tuser,shorthostname,' ',loadcmd]);
