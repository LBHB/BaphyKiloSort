...
global SERVER_PATH BAPHYDATAROOT
...
handles.make_local_data_copy_button = uicontrol('Parent', parent_handle,...
    'Style', 'pushbutton', 'Units', 'pixels',...
    'HorizontalAlignment', 'left', 'String', 'Copy raw data to local machine', ...
    'Max', 1, 'Min', 1, ...
    'Position', [do_buttons_hpos2 ht+1*ts 200 ts], ...
    'Callback', @local_data_copy_callback);
...
%reload_job_table
    ...
    function local_data_copy_callback(Hobj,ev)
        if(isempty(ji))
            return
        end
        this_job=jobs(ji);
        j=load([this_job.pth filesep this_job.name]);
        j.fbinary_local = strrep(j.fbinary, BAPHYDATAROOT, '/KiloSort/');
        UTmkdir(j.fbinary_local);
        fprintf('Copying %s to %s...\n',j.fbinary,j.fbinary_local)
        [s,msg] = copyfile(j.fbinary,j.fbinary_local);
        
        %Change params file to point to local copy
        params_file = [j.results_path filesep 'params.py'];
        A = regexp( fileread(params_file), '\n', 'split');
        A{1} = ['dat_path = ''' j.fbinary_local ''''];
        fid = fopen(params_file, 'w');
        fprintf(fid, '%s\n', A{:});
        fclose(fid);
        
        fprintf('Copied raw data to %s and changed dat_path in %s to point to it.\n',...
                j.fbinary_local,params_file)
        save([this_job.pth filesep this_job.name], '-Struct','j')
    end        
